...
import { PARTY_AGE_LABEL, PARTY_GENDER_LABEL, PARTY_CATEGORY_LABEL } from 'src/constants/options';
import { PartyDetailResponse } from 'types/party/detail/PartyDetailResponse';

type PartyBriefProps = Pick<
    PartyDetailResponse,
    'partyTitle' | 'hit' | 'totalParticipant' | 'participate' | 'gender' | 'age' | 'category'
>;
...
const PartyBrief = (data: PartyBriefProps) => {
    const { partyTitle, hit, totalParticipant, participate, gender, age, category } = data;
    ...
    const PartyBriefData = [
        ...
        },
        {
            id: 'gender_data',
            icon: <GenderIcon />,
            content: `${PARTY_GENDER_LABEL.find((item) => item.value === gender)?.name}`,
        },
        {
            id: 'age_data',
            icon: <InfoIcon />,
        ...
        },
        {
            id: 'category_data',
            icon: <RestaurantIcon />,
            content: `${PARTY_CATEGORY_LABEL.find((item) => item.value === category)?.name}`,
        },
    ];
